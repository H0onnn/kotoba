...
import { useWordSearch, useNewsAnalysis, usePanelHighlight } from '@/app/news/_contexts';
...
export const SummaryPanel = () => {
  ...
  const { result, clearResult, clearSummaryError, isStreaming, streamingSections, streamingCoreSummary, summaryError } =
    useNewsAnalysis();

  const allSections = isStreaming ? streamingSections : result?.summary?.sections || [];

  const coreSummary = isStreaming ? streamingCoreSummary : result?.summary?.coreSummary || '';
  ...
  if (!result && !isStreaming) {
    ...
    return <div className='flex justify-center items-center p-4 h-full text-gray-500'>요약된 내용이 없어요</div>;
  }

  return (
    <LoadingOverlay isLoading={streamingSections.length === 0} loadingText='AI가 기사를 요약 중이에요...'>
      <Card className='h-full rounded-none dark:bg-black'>
        ...
        <CardBody className='overflow-auto flex-1 space-y-4'>
          ...
          >
            ...
            <p className='text-sm'>
              {isStreaming && !result?.metadata.description ? (
                <span className='text-gray-400 animate-pulse'>요약 정보 불러오는 중...</span>
              ) : (
          ...
          >
            <Divider />
            <div>
              <h5 className='my-2 text-sm font-medium text-gray-600'>키워드 단어</h5>
              {isStreaming && !result?.metadata.keywords ? (
                <span className='text-gray-400 animate-pulse'>키워드 불러오는 중...</span>
              ) : (
          ...
          {((result?.summary && result.summary.title) || isStreaming) && (
            ...
            >
              ...
              <h5 className='mt-2 text-sm font-medium text-gray-600'>AI 요약</h5>
              ...
              {(result?.summary?.title || coreSummary) && (
                ...
                >
                  <h6 className='mb-2 font-medium'>{result?.summary?.title || '핵심 요약'}</h6>
                ...
                {allSections.map((section, index) => (
                  <motion.div
                    ...
                    className={`p-4 mb-4 rounded-lg border transition-all duration-200 ${
                      section.relatedParagraphs && section.relatedParagraphs.length > 0
                    ...
                    onClick={() => {
                      if (section.relatedParagraphs) {
                        const paragraphIds = section.relatedParagraphs.map((p) => p.id);
          ...
          {isStreaming && (
            <div className='flex justify-center items-center space-x-2'>
              <Spinner size='sm' />
              <span className='text-gray-400 animate-pulse'>남은 부분 요약 중 ...</span>
            </div>
          )}
        </CardBody>
      </Card>
