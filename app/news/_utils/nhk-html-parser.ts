...
export class NhkHTMLParser {
  ...
  private static extractMainContent(html: string): string {
    ...
    return contentBodyMatches
      ? contentBodyMatches.map((match) => match.replace(this.PATTERNS.CONTENT_BODY, '$1')).join('\n\n')
      : mainMatch[1];
  }
  ...
  private static parseContentToParagraphs(content: string): StructuredParagraph[] {
    ...
    for (const block of blocks) {
      ...
      if (headingMatch) {
        const headingText = this.cleanText(headingMatch[2].replace(this.PATTERNS.HTML_TAGS, ' '));
        ...
        if (remainingContent.trim()) {
          const subParagraphs = this.extractParagraphsFromContent(remainingContent, paragraphId);
  ...
  static async parseFromUrl(url: string, timeout: number = 30000): Promise<ParsedContent> {
    try {
