    ...
    <div className={`max-w-none prose prose-gray ${className}`}>
      <ReactMarkdown
        components={{
          // 헤딩 스타일링
          h1: ({ children }) => (
            <h1 className='pb-4 mb-3 text-2xl font-bold text-gray-900 border-b-2 border-blue-200'>{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className='pb-3 mb-3 text-xl font-semibold text-gray-800 border-b border-gray-200'>{children}</h2>
          ),
          h3: ({ children }) => <h3 className='mb-2 text-lg font-medium text-gray-800'>{children}</h3>,
          h4: ({ children }) => <h4 className='mb-2 text-base font-medium text-gray-700'>{children}</h4>,

          // 문단 스타일링
          p: ({ children }) => <p className='mb-1 leading-relaxed text-gray-700'>{children}</p>,

          // 리스트 스타일링
          ul: ({ children }) => <ul className='mb-4 ml-4 space-y-1 list-disc list-inside'>{children}</ul>,
          ol: ({ children }) => <ol className='mb-4 ml-4 space-y-1 list-decimal list-inside'>{children}</ol>,
          ...
          code: ({ className, children, ...props }) => {
            ...
            return match ? (
              <div className='my-4'>
                <SyntaxHighlighter language={match[1]} style={vs}>
          ...
          td: ({ children }) => (
            <td className='px-4 py-3 text-sm text-gray-700 align-top border-b border-gray-200'>{children}</td>
          ),
